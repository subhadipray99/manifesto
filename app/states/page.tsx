...
export default function StatesIndexPage() {
  ...
  return (
    <div className="min-h-dvh bg-gradient-to-b from-orange-50 to-background">
      {/* Header */}
      <header className="sticky top-0 z-30 border-b border-border bg-gradient-to-r from-orange-600 to-orange-500 px-4 py-4 shadow-lg">
        <div className="mx-auto max-w-2xl">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-t-xl rounded-br-sm bg-white text-3xl font-black text-orange-600" style={{ fontFamily: '"Oswald", sans-serif', fontStyle: 'italic' }}>
              M
            </div>
            <div>
      ...
      <main className="px-4 py-8">
        <div className="mx-auto max-w-2xl">
            ...
            {states.map((state) => {
              ...
              return (
                <Link
                  key={state.id}
                  href={`/${state.id}`}
                  className="group flex items-center justify-between rounded-xl border border-border bg-white p-4 shadow-sm transition-all hover:border-orange-300 hover:shadow-md active:scale-[0.98]"
                >
                  <div className="flex items-center gap-3">
                  ...
                  <ChevronRight className="h-5 w-5 text-muted-foreground transition-transform group-hover:translate-x-0.5 group-hover:text-orange-600" />
                </Link>
              )
            })}
          </div>
          ...
          {states.length === 1 && (
            <div className="mt-8 rounded-xl border border-dashed border-orange-300 bg-orange-50/50 p-6 text-center">
